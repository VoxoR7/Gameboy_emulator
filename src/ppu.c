...
void ppu_disable(void) {
    #ifdef FAST // this should never occur
        if (ppu.state != PPU_VBLANK) {
            #ifdef DEBUG
                LOG_MESG(LOG_FATAL, "LCDC.7 on while ppu not in VBLANK state. A real gameboy might have serious issue.");
            #endif
    ...
    switch (ppu.state) {
        ...
        case PPU_VBLANK:
            ...
            if (ppu.ly >= 153) {
                ...
                #ifdef DEBUG
                static uint16_t debug_display = 100;
                debug_display--;
                if (debug_display == 0) {
                    debug_display = 100;
                    display_try();
        ...
        #ifdef DEBUG
        default:
            LOG_MESG(LOG_FATAL, "PPU not in the good state!");
            exit(EXIT_FAILURE);
        #endif
    }
}