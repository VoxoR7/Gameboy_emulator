...
void memory_init(char *rom) {
    ...
    f = fopen(rom, "r");
    if (f == NULL) {
        LOG_MESG(LOG_FATAL, "The path %s for the rom is wrong!", rom);
        exit(EXIT_FAILURE);
    }
    ...
    switch (emul_cartridge[0x147]) {
        ...
        default:
            #ifdef DEBUG
                LOG_MESG(LOG_FATAL, "Unable to read the rom mbc type (0x%02x)", emul_cartridge[0x147]);
            #endif
            exit(EXIT_FAILURE);
    }
        ...
        if (emul_mbctype == MBC_TYPE_0)
            LOG_MESG(LOG_DEBUG, "The rom MBC type is 0");
        else if (emul_mbctype == MBC_TYPE_1)
            LOG_MESG(LOG_DEBUG, "The rom MBC type is 1, not supported");
        else if (emul_mbctype == MBC_TYPE_2)
            LOG_MESG(LOG_DEBUG, "The rom MBC type is 2, not supported");
        else if (emul_mbctype == MBC_TYPE_5)
            LOG_MESG(LOG_DEBUG, "The rom MBC type is 5, not supported");
    #endif
    ...
    switch (emul_cartridge[0x148]) {
        ...
        case 0x54:
            #ifdef DEBUG
                LOG_MESG(LOG_FATAL, "The rom size of your cartridge is unoficial and not supported by this emulator");
            #endif
            exit(EXIT_FAILURE);
        default:
            #ifdef DEBUG
                LOG_MESG(LOG_FATAL, "Unable to read the total number of rom bank");
            #endif
            exit(EXIT_FAILURE);
    }

    #ifdef DEBUG
        LOG_MESG(LOG_DEBUG, "Cartridge rom size : %"PRIu16" KiB", 16 * emul_total_rom_bank);
    #endif
    ...
    if (emul_current_ram_bak != -1) {
        switch (emul_cartridge[0x149]) {
            ...
            default:
                #ifdef DEBUG
                    LOG_MESG(LOG_FATAL, "Unable to read the total number of ram bank");
                #endif 
                exit(EXIT_FAILURE);
        }
    }
    ...
    emul_current_ram_bak = 0;
        ...
        if (emul_total_rom_bank == 0 || emul_total_rom_bank == -1) 
            LOG_MESG(LOG_DEBUG, "The cartridge has no ram bank");
        else
            LOG_MESG(LOG_DEBUG, "The cartridge has %"PRIu8" KiB of ram", emul_total_rom_bank * 8);
    #endif

    #ifdef DEBUG
        if (emul_cartridge[0x14A] == 0)
            LOG_MESG(LOG_DEBUG, "The cartridge is supposed to be sold in japan");
        else if (emul_cartridge[0x14A] == 1)
            LOG_MESG(LOG_DEBUG, "The cartridge is supposed to be sold in non-japan country");
        else
            LOG_MESG(LOG_WARN, "Unable to read the sold destination of this cartridge");
    #endif
...
void handle_banking(uint16_t addr, uint8_t value) {
    ...
    #ifdef DEBUG
        NO_IMPL
        LOG_MESG(LOG_DEBUG, "(handle_banking) 0x%04x, value 0x%02x\n", addr, value);
    #endif
    ...
    if (addr < 0x8000)
        ...
        memory[addr] = value;
            ...
            if ((value & 0b01000000) == 0)
                LOG_MESG(LOG_WARN, "Write 0 to LCDC.6 not supported");
            else
                LOG_MESG(LOG_WARN, "Write 1 to LCDC.6 not supported");

            if ((value & 0b00100000) != 0)
                /*fprintf(stdout, "[WARNING] write 0 to LCDC.5 not supported");
            else*/
                LOG_MESG(LOG_WARN, "Write 1 to LCDC.5 (Window enable) not supported");

            if ((value & 0b00010000) == 0) {
                LOG_MESG(LOG_WARN, "Write 0 to LCDC.4 ($8800 tile addressing) not supported");
                ppu_addressing_8800();
            } else {
                LOG_MESG(LOG_DEBUG, "Write 1 to LCDC.4 ($8000 tile addressing)");
                ppu_addressing_8000();
            }

            if ((value & 0b00001000) == 0)
                LOG_MESG(LOG_WARN, "Write 0 to LCDC.3 not supported");
            else
                LOG_MESG(LOG_WARN, "Write 1 to LCDC.3 not supported");

            if ((value & 0b00000100) == 0b00000100)
                LOG_MESG(LOG_WARN, "Write 1 to LCDC.2 (Sprite size 8x16) not supported");
            /*else
        ...
        memory[addr] = value;
            ...
            LOG_MESG(LOG_WARN, "(write8) 0x%02x, value 0x%02x", addr, value);
        }
    #endif
...
uint8_t memory_read8(uint16_t addr) {
    ...
    #ifdef DEBUG
        LOG_MESG(LOG_WARN, "(read8) 0x%02x", addr);
    #endif
...
void memory_write16(uint16_t addr, uint16_t value) {
    if (addr >= 0xC000 && addr < 0xDFFF) { // 4KB Work RAM Bank 0 (WRAM)
        ...
        memory[addr] = (uint8_t)(value & 0xFF);
            ...
            LOG_MESG(LOG_WARN, "(write16) 0x%04x; value 0x%02x", addr, value);
        }
    #endif
    return;
}
...
uint16_t memory_read16(uint16_t addr) {
    ...
    #ifdef DEBUG
        LOG_MESG(LOG_WARN, "addr 0x%04x", addr);
    #endif
    return 0;
}
