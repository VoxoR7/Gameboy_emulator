...
void display_init(void) {
    if (SDL_Init(SDL_INIT_EVERYTHING) != 0) {
        LOG_MESG(LOG_FATAL, "Couldn't initialize SDL");
        exit(EXIT_FAILURE);
    }

    if(TTF_Init() != 0) {
        LOG_MESG(LOG_FATAL, "Couldn't initialize TTF");
        SDL_Quit();
        exit(EXIT_FAILURE);
    }
    ...
    if (window == NULL) {
        LOG_MESG(LOG_FATAL, "Couldn't create window: %s", SDL_GetError());
        exit(EXIT_FAILURE);
    }

    renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
    if (renderer == NULL) {
        LOG_MESG(LOG_FATAL, "Couldn't create renderer: %s", SDL_GetError());
        exit(EXIT_FAILURE);
    }
    ...
    if (font == NULL) {
        LOG_MESG(LOG_FATAL, "Couldn't open font");
        exit(EXIT_FAILURE);
    }
...
void display_try(void) {
        ...
        if (window2nd == NULL || renderer2nd == NULL) {
            #ifdef DEBUG
                LOG_MESG(LOG_WARN, "2nd display not up!");
            #endif
    ...
    for (uint16_t j = 0; j < TILE_TO_DRAW; j++) {
        ...
        for (uint8_t i = 0; i < 8; i++) {
            switch (((memory_read8((0x8001 + j * 16) + i * 2) & 0b10000000) >> 7) * 2 + ((memory_read8((0x8000 + j * 16) + i * 2) & 0b10000000) >> 7)) {
                ...
                #ifdef DEBUG
                default:
                    LOG_MESG(LOG_FATAL, "Unknow error");
                    exit(EXIT_FAILURE);
                #endif
            }
